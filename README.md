...
The question is concrete: on a fixed two-epoch budget, does a compact custom architecture, a deeper textbook architecture, or a frozen pre-trained backbone produce the best classifier, and how do they fail when they fail?
...
Confusion matrix and per-class prediction samples are available in the notebook.
...
The 33-image extra test set that ships with PlantVillage was perfectly classified by ResNet-9. All 33 predictions correct. Useful as a smoke test; the validation accuracy on ~17.5k images is the metric to read.
...
│   ├── Rapport_final.pdf           # original French final report
│   └── journal_projet.md           # deployment journal
├── models/
│   └── plant-disease-model.pth     # best ResNet-9 checkpoint, ~26 MB
├── notebooks/
│   └── plant_cv.ipynb              # full training and evaluation notebook
├── src/
│   ├── __init__.py
...
The dataset is downloaded inside the notebook (PlantVillage augmented variant via `gdown`). Training time on a single consumer GPU is approximately 8 minutes for the ResNet-9 run.
...
PyTorch, Torchvision, NumPy, Pillow, Matplotlib, Jupyter. FastAPI, Uvicorn, and Docker for the deprecated inference service.
